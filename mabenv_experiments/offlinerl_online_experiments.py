...
if online_training:
    algo_names = ['BC', 'CQL', 'PLAS', 'PLASWithPerturbation', 'BEAR', 'SAC', 'BCQ', 'CRR', 'AWR', 'AWAC', 'DDPG', 'TD3', 'COMBO', 'MOPO']
    default_loc += '_ONLINE'
else:
    algo_names = ['BC', 'CQL', 'PLAS', 'PLASWithPerturbation', 'BEAR', 'SAC', 'BCQ', 'AWAC', 'DDPG', 'TD3', 'COMBO', 'MOPO']
resume_from = {
    "seed": None,
    "dataset_name": None,
    "algo_name": None,
...
for seed in seeds:
    ...
    for algo_name in algo_names:
        ...
        # ---------------- check for resume, should be put at the start of of most inner loop ----------------
        if algo_name == 'CQL':
            curr_algo = d3rlpy.algos.CQL(q_func_factory='qr', use_gpu=True, scaler = scaler, action_scaler=action_scaler, reward_scaler=reward_scaler) # use Quantile Regression Q function, default was 'mean'

        elif algo_name == 'PLAS':
            curr_algo = d3rlpy.algos.PLAS(q_func_factory='qr', use_gpu=True, scaler = scaler, action_scaler=action_scaler, reward_scaler=reward_scaler) # use Quantile Regression Q function, default was 'mean'
        elif algo_name == 'PLASWithPerturbation':
            curr_algo = d3rlpy.algos.PLASWithPerturbation(q_func_factory='qr', use_gpu=True, scaler = scaler, action_scaler=action_scaler, reward_scaler=reward_scaler) # use Quantile Regression Q function, default was 'mean'
        elif algo_name == 'DDPG':
            curr_algo = d3rlpy.algos.DDPG(use_gpu=True, scaler = scaler, action_scaler=action_scaler, reward_scaler=reward_scaler)
        elif algo_name == 'BC':
            curr_algo = d3rlpy.algos.BC(use_gpu=True, scaler = scaler, action_scaler=action_scaler, reward_scaler=reward_scaler)
        elif algo_name == 'TD3':
            curr_algo = d3rlpy.algos.TD3(use_gpu=True, scaler = scaler, action_scaler=action_scaler, reward_scaler=reward_scaler)
        elif algo_name == 'BEAR':
            curr_algo = d3rlpy.algos.BEAR(use_gpu=True, scaler = scaler, action_scaler=action_scaler, reward_scaler=reward_scaler)
        elif algo_name == 'SAC':
            curr_algo = d3rlpy.algos.SAC(use_gpu=True, scaler = scaler, action_scaler=action_scaler, reward_scaler=reward_scaler)
        elif algo_name == 'BCQ':
            curr_algo = d3rlpy.algos.BCQ(use_gpu=True, scaler = scaler, action_scaler=action_scaler, reward_scaler=reward_scaler)
        elif algo_name == 'CRR':
            curr_algo = d3rlpy.algos.CRR(use_gpu=True, scaler = scaler, action_scaler=action_scaler, reward_scaler=reward_scaler)
        elif algo_name == 'AWR':
            curr_algo = d3rlpy.algos.AWR(use_gpu=True, scaler = scaler, action_scaler=action_scaler, reward_scaler=reward_scaler)
        elif algo_name == 'AWAC':
            curr_algo = d3rlpy.algos.AWAC(use_gpu=True, scaler = scaler, action_scaler=action_scaler, reward_scaler=reward_scaler)
        ...
        if online_training:
            wandb_run = wandb.init(reinit=True, project=project_title, name=acutal_dir, dir=logdir, sync_tensorboard=True)
        else:
        ...
        if algo_name in ['COMBO', 'MOPO']:
            ...
            dynamics = d3rlpy.dynamics.ProbabilisticEnsembleDynamics(learning_rate=1e-4, use_gpu=True, scaler = scaler, action_scaler=action_scaler, reward_scaler=reward_scaler)
            dynamics.fit(feeded_episodes,
                ...
                scorers=scorers)
            if algo_name == 'COMBO':
                curr_algo = d3rlpy.algos.COMBO(dynamics=dynamics, use_gpu=True, scaler = scaler, action_scaler=action_scaler, reward_scaler=reward_scaler)
            elif algo_name == 'MOPO':
                curr_algo = d3rlpy.algos.MOPO(dynamics=dynamics, use_gpu=True, scaler = scaler, action_scaler=action_scaler, reward_scaler=reward_scaler)
            else:
